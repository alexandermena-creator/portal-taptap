...
const VENDEDOR_COLORS = { 'Alexander Mena': '#3b82f6', 'Berenisse López': '#ec4899', 'David Vanegas': '#10b981' };
...
const mapManagerToVendedor = (vRaw) => {
  ...
  if (n.includes('monse') || n.includes('mont') || n.includes('alexander')) return 'Alexander Mena';
  if (n.includes('estefania') || n.includes('estef') || n.includes('berenisse')) return 'Berenisse López';
  if (n.includes('dania') || n.includes('topete') || n.includes('david')) return 'David Vanegas';
  return String(vRaw); 
};
...
const parseMonto = (v) => {
  ...
  const limpiado = String(v).replace(/[^0-9.-]+/g, "");
...
export default function App() {
  ...
  useEffect(() => {
    ...
    init();
    const unsub = onAuthStateChanged(auth, setUserAuth);
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubCitas = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'citas'), (snap) => {
      setCitas(snap.docs.map(doc => ({ id: doc.id, ...doc.data(), semana: doc.data().semana || obtenerRangoSemana(doc.data().fechaCruda) })).sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0)));
    });
  ...
  const handleLogin = (e) => {
    ...
    if (found) { 
      ...
      setIsLoggedIn(true); 
      setLoginError(''); 
      setFiltroVendedor(found.role === 'comercial' ? found.nombre : 'Todos'); 
    }
  ...
  const guardarCita = async (e) => {
    ...
    else await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'citas'), { ...nuevaCita, vendedor: currentUser?.nombre || 'Desconocido', createdAt: Date.now() });
  ...
  const openEditCita = (cita) => {
    ...
    const agRaw = currentUser?.agencias;
    const misAgList = Array.isArray(agRaw) ? agRaw : (agRaw ? String(agRaw).split(',').map(a => a.trim()) : []);
    setIsCustomAgencia(!misAgList.includes(cita.agencia));
    setShowModalCita(true);
  };

  const deleteCita = async (id) => window.confirm('¿Borrar cita?') && await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'citas', id));
  ...
  const handleUpdatePassword = async (e) => {
    ...
    } catch (e) { setMensajePerfil({ tipo:'error', texto:'Error al actualizar' }); }
  };

  const isMaster = currentUser?.role === 'admin' || currentUser?.role === 'manager';
  const misAgenciasArr = useMemo(() => {
    const agRaw = currentUser?.agencias;
    if (Array.isArray(agRaw)) return agRaw;
    return agRaw ? String(agRaw).split(',').map(a => a.trim()) : [];
  }, [currentUser]);
  ...
  const stats = useMemo(() => {
    const tEnv = pFiltradas.reduce((s, p) => s + (p.montoEnviado || 0), 0);
    ...
    const tCom = pFiltradas.reduce((s, p) => String(p.estatus || '').toUpperCase().includes('COMMITTED') ? s + (p.montoEnviado || 0) : s, 0);
    
    // Conteo por agencia para las citas
    const agMapCitas = cFiltradas.reduce((acc, c) => {
      const a = c.agencia || 'Sin Agencia';
      acc[a] = (acc[a] || 0) + 1;
      return acc;
    }, {});
    const citasBreakdown = Object.entries(agMapCitas).map(([ag, count]) => `${count} ${ag}`).join(', ') || '0 citas';

    const estMap = pFiltradas.reduce((acc, p) => { const e = (p.estatus || 'Sin Estatus').toUpperCase(); acc[e] = (acc[e] || 0) + (p.montoEnviado || 0); return acc; }, {});
    ...
    const cData = target.map(n => ({
      name: (n || '').split(' ')[0],
      monto: pFiltradas.filter(p => p.vendedor === n).reduce((s, p) => s + (Number(p.montoEnviado) || 0), 0),
      cant: pFiltradas.filter(p => p.vendedor === n).length,
      citas: cFiltradas.filter(c => c.vendedor === n).length
    }));
    return { tEnv, tWon, tCom, bateo: tEnv > 0 ? ((tWon / tEnv) * 100).toFixed(1) : 0, pieD, cData, countCitas: cFiltradas.length, citasBreakdown };
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-300">
        <img src="/logo.png" alt="TapTap" className="h-32 mx-auto mb-8 object-contain drop-shadow-sm" />
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-hidden">
      <aside className="w-full md:w-72 bg-slate-950 text-white p-6 flex flex-col shrink-0 border-r border-slate-800">
        <div className="flex items-center justify-center md:justify-start mb-12 mt-4"><img src="/logo.png" alt="TapTap" className="h-20 object-contain invert brightness-0 transition-all hover:scale-105" style={{ filter: 'brightness(0) invert(1)' }} /></div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-800 space-y-4">
          <div className="flex items-center gap-3 bg-slate-900 p-3 rounded-2xl border border-slate-800 shadow-inner"><div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center font-black text-sm text-white shrink-0 shadow-md">{String(currentUser?.nombre || '?').charAt(0)}</div><div className="overflow-hidden text-left"><p className="text-xs font-bold text-white truncate">{currentUser?.nombre}</p><p className="text-[9px] text-slate-400 font-black uppercase tracking-widest truncate mt-0.5">{currentUser?.cargo}</p></div></div>
          <button onClick={() => { setIsLoggedIn(false); setCurrentUser(null); }} className="flex items-center justify-center gap-2 text-xs font-black text-rose-400 hover:text-rose-300 hover:bg-rose-950/30 transition-all w-full p-3 rounded-xl border border-transparent hover:border-rose-900/50"><LogOut size={14} /> SALIR</button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto bg-slate-50">
        <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="text-left"><h2 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 mb-2 flex items-center gap-2">Hola, {String(currentUser?.nombre || '').split(' ')[0]} <span>👋</span></h2><p className="text-slate-500 text-sm font-medium">Asignación: <span className="text-blue-600 font-bold">{currentUser?.agencias}</span></p></div>
          <div className="w-full md:w-auto flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
            <div className="relative w-full sm:w-auto"><button onClick={() => setIsDatePickerOpen(!isDatePickerOpen)} className="flex items-center justify-center gap-2 w-full sm:w-auto bg-white border border-slate-200 text-slate-700 px-6 py-4 rounded-2xl shadow-sm hover:bg-blue-50 transition-all font-bold text-sm outline-none"><Calendar size={16} className={filtroFechaInicio || filtroFechaFin ? "text-blue-600" : "text-slate-400"} />{filtroFechaInicio && filtroFechaFin ? `${formatearFechaCorta(filtroFechaInicio)} - ${formatearFechaCorta(filtroFechaFin)}` : 'Filtrar Fechas'}</button>
              {isDatePickerOpen && (<div className="absolute top-full right-0 lg:left-0 mt-2 p-6 bg-white border border-slate-100 rounded-[2rem] shadow-2xl z-50 w-full sm:w-80 animate-in slide-in-from-top-2"><div className="flex justify-between items-center mb-5"><h4 className="font-black text-slate-900">Rango de Fechas</h4><button onClick={() => setIsDatePickerOpen(false)} className="text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 p-2 rounded-full transition-colors"><X size={16}/></button></div><div className="space-y-4"><div><label className="text-[10px] font-black uppercase text-slate-400 block mb-2">Inicio</label><input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold text-sm [color-scheme:light]" value={filtroFechaInicio} onChange={e => setFiltroFechaInicio(e.target.value)} onClick={(e) => e.target.showPicker && e.target.showPicker()} /></div><div><label className="text-[10px] font-black uppercase text-slate-400 block mb-2">Fin</label><input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold text-sm [color-scheme:light]" value={filtroFechaFin} onChange={e => setFiltroFechaFin(e.target.value)} onClick={(e) => e.target.showPicker && e.target.showPicker()} /></div>{(filtroFechaInicio || filtroFechaFin) && <button onClick={() => {setFiltroFechaInicio(''); setFiltroFechaFin(''); setIsDatePickerOpen(false);}} className="w-full mt-4 text-rose-600 bg-rose-50 border border-rose-100 font-bold text-sm py-3 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm">Limpiar</button>}</div></div>)}
            </div>
            {isMaster && (<select className="w-full sm:w-auto bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 shadow-sm outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer" value={filtroVendedor} onChange={e => setFiltroVendedor(e.target.value)}><option value="Todos">🚀 Todo el Equipo</option>{COMERCIALES.map(c => <option key={c} value={c}>{c}</option>)}</select>)}
          </div>
        </header>
        ...
        {activeTab === 'dashboard' && (
          <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-wrap gap-6 items-stretch">
              ...
              <KpiCard icon={Calendar} color="amber" label="Citas" value={stats.countCitas} subValue={stats.citasBreakdown} />
            </div>
            <div className={`grid grid-cols-1 ${isMaster ? 'xl:grid-cols-2' : 'grid-cols-1'} gap-8`}>
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col items-center"><h3 className="font-bold text-slate-900 mb-2 flex items-center gap-2 w-full"><PieChartIcon size={18} className="text-purple-500"/> Estatus del Pipe</h3><div className="w-full flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.pieD} innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value" label={renderPieLabel} labelLine={false}>{stats.pieD.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip formatter={(v) => formatCurrency(v)} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} /><Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{fontSize: '11px', fontWeight: 'bold'}} /></PieChart></ResponsiveContainer></div></div>
              {isMaster && (<><div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col"><h3 className="font-bold text-slate-900 mb-6 flex items-center gap-2"><TrendingUp size={18} className="text-blue-500"/> Revenue por Comercial</h3><div className="flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.cData} margin={{top: 10, right: 10, left: 0, bottom: 0}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} tickFormatter={(v) => `$${v >= 1000000 ? v/1000000 + 'M' : v/1000 + 'k'}`} width={60} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} formatter={(v) => formatCurrency(v)} /><Bar dataKey="monto" radius={[6, 6, 0, 0]} maxBarSize={60}>{stats.cData.map((e, i) => <Cell key={`cell-${i}`} fill={VENDEDOR_COLORS[e.name + ' Mena'] || VENDEDOR_COLORS[e.name + ' López'] || VENDEDOR_COLORS[e.name + ' Vanegas'] || '#94a3b8'} />)}</Bar></BarChart></ResponsiveContainer></div></div><div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col"><h3 className="font-bold text-slate-900 mb-6 flex items-center gap-2"><FileText size={18} className="text-emerald-500"/> Volumen de Propuestas</h3><div className="flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.cData} margin={{top: 10, right: 10, left: 0, bottom: 0}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} allowDecimals={false} width={30} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} /><Bar dataKey="cant" radius={[6, 6, 0, 0]} maxBarSize={60}>{stats.cData.map((e, i) => <Cell key={`cell-${i}`} fill={VENDEDOR_COLORS[e.name + ' Mena'] || VENDEDOR_COLORS[e.name + ' López'] || VENDEDOR_COLORS[e.name + ' Vanegas'] || '#94a3b8'} />)}</Bar></BarChart></ResponsiveContainer></div></div><div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col"><h3 className="font-bold text-slate-900 mb-6 flex items-center gap-2"><Calendar size={18} className="text-amber-500"/> Citas Registradas</h3><div className="flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.cData} margin={{top: 10, right: 10, left: 0, bottom: 0}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} allowDecimals={false} width={30} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} /><Bar dataKey="citas" radius={[6, 6, 0, 0]} maxBarSize={60}>{stats.cData.map((e, i) => <Cell key={`cell-${i}`} fill={VENDEDOR_COLORS[e.name + ' Mena'] || VENDEDOR_COLORS[e.name + ' López'] || VENDEDOR_COLORS[e.name + ' Vanegas'] || '#94a3b8'} />)}</Bar></BarChart></ResponsiveContainer></div></div></>)}
            </div>
          </div>
        )}
        ...
        {activeTab === 'pipe' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
            <div className="flex justify-between items-center"><h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Pipe de Drive</h2><button onClick={() => downloadCSV(pFiltradas, 'Pipe')} className="bg-white border border-slate-200 text-slate-700 font-bold px-5 py-3 rounded-xl flex items-center gap-2 shadow-sm hover:bg-slate-50 transition-all text-sm shadow-md"><Download size={16} /> CSV</button></div>
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden relative"><div className="max-h-[600px] overflow-auto w-full relative"><table className="w-full text-left min-w-[800px]"><thead className="bg-slate-50 border-b border-slate-200 text-[10px] font-black uppercase text-slate-400 tracking-widest sticky top-0 z-10"><tr><th className="p-5">Fecha / Sem.</th><th className="p-5">Campaña</th><th className="p-5">Manager</th><th className="p-5 text-center">Estatus</th><th className="p-5 text-right">Monto</th></tr></thead><tbody className="divide-y divide-slate-100 text-sm">{pFiltradas.map(p => (<tr key={p.id} className="hover:bg-slate-50/50 transition"><td className="p-5"><div className="text-slate-500 font-medium">{p.fechaCruda}</div><div className="text-[10px] text-slate-400 font-bold uppercase mt-1">{p.semana}</div></td><td className="p-5 font-bold text-slate-800">{p.nombre}</td><td className="p-5"><span className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-[10px] font-black uppercase border border-blue-100">{p.vendedor}</span></td><td className="p-5 text-center"><span className={`px-2.5 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest ${String(p.estatus || '').toUpperCase().includes('WON') ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : String(p.estatus || '').toUpperCase().includes('COMMITTED') ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : String(p.estatus || '').toUpperCase().includes('LOST') ? 'bg-rose-100 text-rose-700 border border-rose-200' : 'bg-slate-100 text-slate-700 border border-slate-200'}`}>{p.estatus || 'Enviada'}</span></td><td className="p-5 font-black text-slate-900 text-right">{formatCurrency(p.montoEnviado)}</td></tr>))}</tbody></table></div></div>
          </div>
        )}

        {activeTab === 'citas' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
            <header className="flex justify-between items-center"><h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Agenda Semanal</h2><div className="flex gap-3"><button onClick={() => downloadCSV(cFiltradas, 'Agenda')} className="bg-white border border-slate-200 text-slate-700 font-bold px-6 py-4 rounded-2xl shadow-sm hover:bg-slate-50 transition-all text-sm shadow-md"><Download size={16} /> CSV</button><button onClick={() => {setEditingCitaId(null); setNuevaCita({ agencia: '', vendedor: '', fechaCruda: '', semana: '', persona: '', cuenta: '' }); setIsCustomAgencia(false); setShowModalCita(true);}} className="bg-blue-600 text-white font-black px-6 py-4 rounded-2xl shadow-xl shadow-blue-600/20 hover:scale-105 transition-all"><Plus size={20} /> Registrar</button></div></header>
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden relative"><div className="max-h-[600px] overflow-auto w-full relative"><table className="w-full text-left min-w-[900px]"><thead className="bg-slate-50 border-b border-slate-200 text-[10px] font-black uppercase text-slate-400 tracking-widest sticky top-0 z-10"><tr><th className="p-5">Semana</th><th className="p-5">Agencia</th><th className="p-5">Cuenta / Marca</th><th className="p-5">Manager</th><th className="p-5">Contacto</th><th className="p-5">Fecha</th><th className="p-5 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100 text-sm">{cFiltradas.length === 0 ? (<tr><td colSpan="7" className="p-10 text-center font-bold text-slate-400">No hay citas registradas.</td></tr>) : cFiltradas.map(c => (
              <tr key={c.id} className="hover:bg-slate-50 transition-colors"><td className="p-5"><span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-black uppercase">{c.semana}</span></td><td className="p-5 font-bold text-slate-900">{c.agencia}</td><td className="p-5 font-medium text-slate-600">{c.cuenta}</td><td className="p-5"><span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-lg text-[10px] font-black">{c.vendedor}</span></td><td className="p-5 text-slate-500">{c.persona}</td><td className="p-5 text-slate-400 font-medium">{c.fechaCruda}</td><td className="p-5 text-right"><div className="flex justify-end gap-2">{(currentUser?.role === 'admin' || currentUser?.nombre === c.vendedor) && <><button onClick={() => openEditCita(c)} className="p-2 text-slate-400 hover:text-blue-600 transition-all"><Edit3 size={16}/></button><button onClick={() => deleteCita(c.id)} className="p-2 text-slate-400 hover:text-rose-600 transition-all"><Trash2 size={16}/></button></>}</div></td></tr>))}</tbody></table></div></div>
          </div>
        )}

        {activeTab === 'perfil' && currentUser && (
          <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
            <header className="mb-2"><h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Mi Perfil</h2><p className="text-slate-500 font-medium mt-1">Gestión de seguridad y asignaciones.</p></header>
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-10 pb-10 border-b border-slate-100"><div className="w-24 h-24 rounded-[2rem] bg-blue-50 text-blue-600 flex items-center justify-center font-black text-4xl shrink-0 shadow-inner border border-blue-100">{currentUser?.nombre?.charAt(0)}</div><div><h3 className="text-3xl font-black text-slate-900">{currentUser?.nombre}</h3><p className="text-blue-600 font-bold uppercase tracking-widest text-sm mt-1">{currentUser?.cargo}</p><span className="inline-block mt-3 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-slate-200 bg-slate-50 text-slate-700">Nivel: {currentUser?.role}</span></div></div>
              <div className="space-y-10">
                <div><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Building2 size={14}/> Mis Agencias</h4><div className="flex flex-wrap gap-2">{misAgenciasArr.map(a => <span key={a} className="bg-slate-50 border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm">{a}</span>)}{!misAgenciasArr.length && <span className="text-slate-400 italic">No tienes agencias asignadas.</span>}</div></div>
                <div className="pt-10 border-t border-slate-100"><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Lock size={14}/> Nueva Contraseña</h4><form onSubmit={handleUpdatePassword} className="flex flex-col sm:flex-row gap-4"><input type="text" required placeholder="Mínimo 6" value={nuevaContra} onChange={e => setNuevaContra(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-4 font-bold text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500" /><button type="submit" className="w-full sm:w-auto bg-slate-900 text-white font-black px-8 py-4 rounded-2xl shadow-lg hover:bg-blue-600 transition-all uppercase tracking-widest text-xs whitespace-nowrap">Guardar</button></form>{mensajePerfil.texto && <div className={`mt-4 p-4 rounded-xl text-sm font-bold flex items-center gap-2 ${mensajePerfil.tipo === 'exito' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>{mensajePerfil.texto}</div>}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'admin' && currentUser?.role === 'admin' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300"><header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4"><div><h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Control Maestro</h2><p className="text-slate-500 font-medium mt-1">Accesos, Cargos y Agencias.</p></div><button onClick={() => { setEditingUser(null); setFormUser({ nombre: '', pass: '', role: 'comercial', cargo: '', agencias: '' }); setShowModalUser(true); }} className="bg-slate-900 text-white font-black px-6 py-4 rounded-2xl flex items-center gap-2 shadow-xl hover:scale-105 transition-all"><Plus size={20} /> Nuevo Perfil</button></header><div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden relative"><div className="overflow-x-auto w-full"><table className="w-full text-left min-w-[800px] whitespace-nowrap md:whitespace-normal"><thead className="bg-slate-50 border-b border-slate-100"><tr className="text-[10px] font-black uppercase text-slate-400 tracking-widest"><th className="p-5">Colaborador / Cargo</th><th className="p-5">Asignación</th><th className="p-5">Pass</th><th className="p-5">Rol / Nivel</th><th className="p-5 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100">{usuarios.map(u => (<tr key={u.id} className="hover:bg-slate-50/50 transition"><td className="p-5"><div className="font-bold text-slate-900">{u.nombre}</div><div className="text-[10px] font-black text-blue-500 uppercase tracking-tighter mt-1">{u.cargo}</div></td><td className="p-5 text-xs text-slate-500 font-medium">{u.agencias}</td><td className="p-5 font-mono text-slate-400 text-[10px] tracking-widest bg-slate-50 rounded p-1 mx-2">{u.pass}</td><td className="p-5"><span className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${u.role === 'admin' ? 'bg-rose-50 text-rose-700 border-rose-100' : u.role === 'manager' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{u.role}</span></td><td className="p-5 text-right space-x-2"><div className="flex justify-end gap-1"><button onClick={() => { setEditingUser(u); setFormUser(u); setShowModalUser(true); }} className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-blue-600 shadow-sm transition-all"><Edit3 size={16}/></button><button onClick={() => deleteUser(u.id)} className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-rose-600 shadow-sm transition-all"><Trash2 size={16}/></button></div></td></tr>))}</tbody></table></div></div></div>
        )}
      </main>
      ...
      {showModalCita && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200"><div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden p-8 border border-slate-100"><div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black text-slate-900 tracking-tight">{editingCitaId ? 'Editar Cita' : 'Agendar Cita'}</h3><button onClick={() => {setShowModalCita(false); setEditingCitaId(null);}} className="text-slate-400 bg-slate-50 hover:bg-rose-50 hover:text-rose-500 p-2 rounded-full transition-colors"><X size={20}/></button></div><form onSubmit={guardarCita} className="space-y-5">
          <div className="space-y-1 text-left"><label className="text-[10px] font-black uppercase text-slate-400 ml-2 block mb-1">Agencia</label>{!isCustomAgencia ? (<select required className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-sm" value={nuevaCita.agencia} onChange={e => e.target.value === 'otra' ? setIsCustomAgencia(true) : setNuevaCita({...nuevaCita, agencia: e.target.value})}><option value="">Selecciona...</option>{misAgenciasArr.map(a => <option key={a} value={a}>{a}</option>)}<option value="otra">+ Otra...</option></select>) : (<div className="flex gap-2"><input required className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-sm" value={nuevaCita.agencia} onChange={e => setNuevaCita({...nuevaCita, agencia: e.target.value})} /><button type="button" onClick={() => setIsCustomAgencia(false)} className="bg-rose-50 text-rose-500 px-4 rounded-xl font-bold"><X size={16}/></button></div>)}</div>
...
function KpiCard({ icon: Icon, color, label, value, subValue }) {
  ...
  return (
    <div className="flex-1 min-w-[280px] bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 flex flex-col justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div>
        <div className="text-2xl md:text-[23px] lg:text-[25px] xl:text-[28px] font-black text-slate-900 tracking-tighter leading-none mb-1">{value}</div>
        {subValue && <div className="text-[10px] font-bold text-slate-400 leading-tight italic">{subValue}</div>}
      </div>
    </div>
  );
}
...
function InputGroup({ label, value, onChange, placeholder = "" }) {
  return (<div className="space-y-1 text-left"><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest block mb-1">{label}</label><input required className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-sans" placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} /></div>);
}