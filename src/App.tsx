...
export default function App() {
  ...
  const openEditCita = (cita) => {
    ...
    const misAgList = currentUser?.agencias ? String(currentUser.agencias).split(',').map(a => a.trim()) : [];
  ...
  const currentAgenciasArr = currentUser?.agencias ? String(currentUser.agencias).split(',').map(a => a.trim()) : [];
  ...
  const stats = useMemo(() => {
    ...
    const cData = target.map(n => ({
      name: (n || '').split(' ')[0],
      monto: pFiltradas.filter(p => p.vendedor === n).reduce((s, p) => s + (p.montoEnviado || 0), 0),
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-300">
        <img src="/logo.png" alt="TapTap" className="h-28 mx-auto mb-8 object-contain drop-shadow-sm" />
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto bg-slate-50">
        <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="text-left"><h2 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 mb-2 flex items-center gap-2">Hola, {(currentUser?.nombre || '').split(' ')[0]} <span>👋</span></h2><p className="text-slate-500 text-sm font-medium">Asignación: <span className="text-blue-600 font-bold">{currentUser?.agencias}</span></p></div>
        ...
        {activeTab === 'dashboard' && (
          <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-wrap gap-6 items-stretch"><KpiCard icon={Clock} color="blue" label="Revenue Pipe" value={formatCurrency(stats.tEnv)} /><KpiCard icon={Target} color="indigo" label="Committed" value={formatCurrency(stats.tCom)} /><KpiCard icon={CheckCircle2} color="green" label="Total WON" value={formatCurrency(stats.tWon)} /><KpiCard icon={Activity} color="purple" label="Tasa Bateo" value={`${stats.bateo}%`} /><KpiCard icon={Calendar} color="amber" label="Citas Activas" value={stats.countCitas} /></div>
        ...
        {activeTab === 'perfil' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
            <header className="mb-2"><h2 className="text-3xl font-black text-slate-900 tracking-tight">Mi Perfil</h2></header>
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
              <div className="flex items-center gap-6 mb-10 pb-10 border-b border-slate-100"><div className="w-24 h-24 rounded-[2rem] bg-blue-50 text-blue-600 flex items-center justify-center font-black text-4xl shrink-0">{currentUser?.nombre?.charAt(0)}</div><div><h3 className="text-3xl font-black text-slate-900">{currentUser?.nombre}</h3><p className="text-blue-600 font-bold uppercase tracking-widest text-sm mt-1">{currentUser?.cargo}</p><span className="inline-block mt-3 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border bg-slate-50 text-slate-700">Nivel: {currentUser?.role}</span></div></div>
              <div className="space-y-10">
                <div><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Building2 size={14}/> Agencias</h4><div className="flex flex-wrap gap-2">{misAgenciasArr.map(a => <span key={a} className="bg-slate-50 border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm">{a}</span>)}</div></div>
                <div className="pt-10 border-t border-slate-100"><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Lock size={14}/> Nueva Contraseña</h4><form onSubmit={handleUpdatePassword} className="flex flex-col sm:flex-row gap-4"><input type="text" required placeholder="Mínimo 6" value={nuevaContra} onChange={e => setNuevaContra(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-4 font-bold text-sm text-slate-700" /><button type="submit" className="bg-slate-900 text-white font-black px-8 py-4 rounded-2xl shadow-lg hover:bg-blue-600 transition-all uppercase tracking-widest text-xs">Guardar</button></form>{mensajePerfil.texto && <div className={`mt-4 p-4 rounded-xl text-sm font-bold flex items-center gap-2 ${mensajePerfil.tipo === 'exito' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{mensajePerfil.texto}</div>}</div>
              </div>
            </div>
          </div>
        )}
...
function KpiCard({ icon: Icon, color, label, value }) {
  ...
  return (
    <div className="flex-1 min-w-[240px] xl:min-w-[210px] bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 flex flex-col justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
