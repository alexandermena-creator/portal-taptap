...
export default function App() {
  ...
  const openEditCita = (cita) => {
    ...
    const misAgenciasList = currentUser?.agencias ? String(currentUser.agencias).split(',').map(a => a.trim()) : [];
    setIsCustomAgencia(!misAgenciasList.includes(cita.agencia));
  ...
  const misAgenciasArr = currentUser?.agencias ? String(currentUser.agencias).split(',').map(a => a.trim()) : [];
  ...
  const stats = useMemo(() => {
    const tEnv = pFiltradas.reduce((s, p) => s + (Number(p.montoEnviado) || 0), 0);
    const tWon = pFiltradas.reduce((s, p) => {
        const st = String(p.estatus || '').toUpperCase();
        if (st.includes('WON') || st === 'CERRADA') return s + (Number(p.montoCerrado) || Number(p.montoEnviado) || 0);
        return s;
    }, 0);
    const tCom = pFiltradas.reduce((s, p) => String(p.estatus || '').toUpperCase().includes('COMMITTED') ? s + (Number(p.montoEnviado) || 0) : s, 0);
    
    const estMap = pFiltradas.reduce((acc, p) => { 
        const e = (p.estatus || 'Sin Estatus').toUpperCase(); 
        acc[e] = (acc[e] || 0) + (Number(p.montoEnviado) || 0); 
        return acc; 
    }, {});
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-300">
        <img src="/logo.png" alt="TapTap" className="h-24 md:h-32 mx-auto mb-8 object-contain" />
        ...
        <form onSubmit={handleLogin} className="space-y-5 text-left">
          <div>
            ...
            <select className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all cursor-pointer" value={loginForm.user} onChange={e => setLoginForm({...loginForm, user: e.target.value})} required>
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 pl-12" value={loginForm.pass} onChange={e => setLoginForm({...loginForm, pass: e.target.value})} required />
              <Lock className="absolute left-4 top-4 text-slate-400" size={20} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-4 text-slate-300 hover:text-slate-700">{showPassword ? <EyeOff size={20}/> : <Eye size={20}/>}</button>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-hidden">
      <aside className="w-full md:w-72 bg-slate-950 text-white p-6 flex flex-col shrink-0 border-r border-slate-800">
        <div className="flex items-center justify-center md:justify-start mb-12 mt-4">
          <img src="/logo.png" alt="TapTap" className="h-14 md:h-18 lg:h-20 object-contain invert brightness-0 transition-all hover:scale-105" style={{ filter: 'brightness(0) invert(1)' }} />
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-800 space-y-4">
          <div className="flex items-center gap-3 bg-slate-900 p-3 rounded-2xl border border-slate-800 shadow-inner">
            ...
            <div className="overflow-hidden text-left">
              ...
              <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest truncate mt-0.5">{currentUser?.cargo}</p>
            </div>
          </div>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto bg-slate-50">
        <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="text-left">
            <h2 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 mb-2 flex items-center gap-2">
              Hola, {(currentUser?.nombre || '').split(' ')[0]} <span>👋</span>
            </h2>
            ...
            <div className="relative w-full sm:w-auto">
              ...
              {isDatePickerOpen && (
                <div className="absolute top-full right-0 lg:left-0 mt-2 p-6 bg-white border border-slate-100 rounded-[2rem] shadow-2xl z-50 w-full sm:w-80 animate-in slide-in-from-top-2">
                  <div className="flex justify-between items-center mb-5"><h4 className="font-black text-slate-900">Rango de Fechas</h4><button onClick={() => setIsDatePickerOpen(false)} className="text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 p-2 rounded-full transition-colors"><X size={16}/></button></div>
              ...
              {isMaster && (
                <>
                  ...
                  <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col">
                    ...
                    <div className="flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.cData} margin={{top: 10, right: 10, left: 0, bottom: 0}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} allowDecimals={false} width={30} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} /><Bar dataKey="cant" radius={[6, 6, 0, 0]} maxBarSize={60}>{stats.cData.map((e, i) => <Cell key={`cell-${i}`} fill={VENDEDOR_COLORS[e.name + ' Mena'] || VENDEDOR_COLORS[e.name + ' López'] || VENDEDOR_COLORS[e.name + ' Vanegas'] || '#94a3b8'} />)}</Bar></BarChart></ResponsiveContainer></div>
                  </div>
                  <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col">
                    ...
                    <div className="flex-1 min-h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.cData} margin={{top: 10, right: 10, left: 0, bottom: 0}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} allowDecimals={false} width={30} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '16px', border: 'none', fontWeight: 'bold'}} /><Bar dataKey="citas" radius={[6, 6, 0, 0]} maxBarSize={60}>{stats.cData.map((e, i) => <Cell key={`cell-${i}`} fill={VENDEDOR_COLORS[e.name + ' Mena'] || VENDEDOR_COLORS[e.name + ' López'] || VENDEDOR_COLORS[e.name + ' Vanegas'] || '#94a3b8'} />)}</Bar></BarChart></ResponsiveContainer></div>
                  </div>
                </>
              )}
            </div>
        ...
        {activeTab === 'citas' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden relative"><div className="max-h-[600px] overflow-auto w-full relative"><table className="w-full text-left min-w-[900px]"><thead className="bg-slate-50 border-b border-slate-200 text-[10px] font-black uppercase text-slate-400 tracking-widest sticky top-0 z-10 shadow-sm"><tr><th className="p-5">Semana</th><th className="p-5">Agencia</th><th className="p-5">Cuenta / Marca</th><th className="p-5">Manager</th><th className="p-5">Contacto</th><th className="p-5">Fecha</th><th className="p-5 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100 text-sm">{cFiltradas.length === 0 ? (<tr><td colSpan="7" className="p-10 text-center text-slate-400 font-bold">No hay citas.</td></tr>) : citasFiltradas.map(c => (
        ...
        {activeTab === 'perfil' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
            <header className="mb-2"><h2 className="text-3xl font-black text-slate-900 tracking-tight">Mi Perfil</h2><p className="text-slate-500 font-medium mt-1">Seguridad y Datos.</p></header>
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-10 pb-10 border-b border-slate-100"><div className="w-24 h-24 rounded-[2rem] bg-blue-50 text-blue-600 flex items-center justify-center font-black text-4xl shadow-inner shrink-0">{currentUser?.nombre?.charAt(0)}</div><div><h3 className="text-3xl font-black text-slate-900">{currentUser?.nombre}</h3><p className="text-blue-600 font-bold uppercase tracking-widest text-sm mt-1">{currentUser?.cargo}</p><span className={`inline-block mt-3 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${currentUser?.role === 'admin' ? 'bg-rose-50 text-rose-700 border-rose-100' : currentUser?.role === 'manager' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>Nivel: {currentUser?.role}</span></div></div>
              <div className="space-y-10">
                <div><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Building2 size={14}/> Mis Agencias</h4><div className="flex flex-wrap gap-2">{misAgenciasArr.map(a => <span key={a} className="bg-slate-50 border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm">{a}</span>)}{!misAgenciasArr.length && <span className="text-slate-400 italic">No asignadas.</span>}</div></div>
                <div className="pt-10 border-t border-slate-100"><h4 className="text-[11px] font-black uppercase text-slate-400 tracking-widest block mb-4 flex items-center gap-2"><Lock size={14}/> Nueva Contraseña</h4><form onSubmit={handleUpdatePassword} className="flex flex-col sm:flex-row gap-4"><input type="text" required placeholder="Nueva contraseña (mín. 6)" value={nuevaContra} onChange={e => setNuevaContra(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-4 font-bold text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500" /><button type="submit" className="w-full sm:w-auto bg-slate-900 text-white font-black px-8 py-4 rounded-2xl shadow-lg hover:bg-blue-600 transition-all uppercase tracking-widest text-xs whitespace-nowrap">Guardar</button></form>{mensajePerfil.texto && <div className={`mt-4 p-4 rounded-xl text-sm font-bold flex items-center gap-2 ${mensajePerfil.tipo === 'exito' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>{mensajePerfil.tipo === 'exito' ? <CheckCircle2 size={16}/> : <X size={16}/>}{mensajePerfil.texto}</div>}</div>
              </div>
            </div>
          </div>
        )}
...
function KpiCard({ icon: Icon, color, label, value }) {
  ...
  return (
    <div className="flex-1 min-w-[240px] bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 flex flex-col justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
